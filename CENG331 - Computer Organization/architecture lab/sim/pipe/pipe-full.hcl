...
# While determining next PC, if M_icode == IJTAB, then f_pc becomes m_valM. Because we update the PC with valM.
# I have added IJTAB in instr_valid, need_regids, need_valC because our instruction takes 10 bytes as argument: The first byte is icode and ifun, the second byte is registers, and bytes between 3-10 are the constant.
# I have not added IJTAB in d_srcA because the first register is null register.
# I have added IJTAB in d_srcB because we read the data in the second register and store it at valB.
# aluA is E_valC, aluB is E_valB because we do the addition of valB and valC in execute stage.
# mem_addr is M_valE because we read M[valE] and store it in valM in memory stage.
# IJTAB is included in mem_read because this instruction does memory read operation.
# jtab can have load-use hazards because, for example if a register R is loaded just before the jtab instruction, and if the rB of jtab is R, then there is a load-use hazard.
# Since there can be load-use hazards, my pipeline register control is the same as other instructions that have load-use hazards:
# F -> stall
# D -> stall
# E -> bubble
# M -> normal
...
word f_pc = [
	...
	W_icode == IRET : W_valM;
	M_icode == IJTAB : m_valM;		# ADDED IJTAB
...
bool F_stall =
	...
	E_icode in { IMRMOVQ, IPOPQ, IJTAB} &&		# ADDED IJTAB
...
bool D_bubble =
	...
	!(E_icode in { IMRMOVQ, IPOPQ, IJTAB } && E_dstM in { d_srcA, d_srcB }) &&	# ADDED IJTAB
  IRET in { D_icode, E_icode, M_icode };		
