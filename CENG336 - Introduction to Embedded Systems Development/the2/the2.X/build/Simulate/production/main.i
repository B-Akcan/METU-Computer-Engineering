...
Rotation rotation;
Type type;
unsigned char PORTA_previous[ARROW_KEY_COUNT];
unsigned char PORTA_current[ARROW_KEY_COUNT];

void busy_wait();
void blink();
void init();
unsigned char not_intersects();
void draw();
void rotate();
void submit();
unsigned char canMove(Position position);
...
void busy_wait() {
    for (long int i = 0; i <= 454544; i++);
    INTCON |= (1 << 7);
}

unsigned char blink_on = 1;
...
void HandleInterrupt() {
    ...
    if (((INTCON) & (1 << (0))))
    {
        if (((PORTB) & (1 << (5))))
            rotate();
        if (!prev_submit && ((PORTB) & (1 << (6)))) {
            submit();
        }
        prev_submit = ((PORTB) & (1 << (6)));
    }
...
void setLed(Position pos, unsigned char value) {
    switch (pos.x) {
        case 0:
            if (value)
                LATC |= (1 << pos.y);
...
void draw() {
    LATC = (unsigned char) ((grid[0][0] << 0) | (grid[0][1] << 1) | (grid[0][2] << 2) | (grid[0][3] << 3) | (grid[0][4] << 4) | (grid[0][5] << 5) | (grid[0][6] << 6) | (grid[0][7] << 7));
    LATD = (unsigned char) ((grid[1][0] << 0) | (grid[1][1] << 1) | (grid[1][2] << 2) | (grid[1][3] << 3) | (grid[1][4] << 4) | (grid[1][5] << 5) | (grid[1][6] << 6) | (grid[1][7] << 7));
    LATE = (unsigned char) ((grid[2][0] << 0) | (grid[2][1] << 1) | (grid[2][2] << 2) | (grid[2][3] << 3) | (grid[2][4] << 4) | (grid[2][5] << 5) | (grid[2][6] << 6) | (grid[2][7] << 7));
    LATF = (unsigned char) ((grid[3][0] << 0) | (grid[3][1] << 1) | (grid[3][2] << 2) | (grid[3][3] << 3) | (grid[3][4] << 4) | (grid[3][5] << 5) | (grid[3][6] << 6) | (grid[3][7] << 7));
...
void poll_a() {
    PORTA_current[KEY_RIGHT] = ((PORTA) & (1 << (KEY_RIGHT)));
    PORTA_current[KEY_UP] = ((PORTA) & (1 << (KEY_UP)));
    PORTA_current[KEY_DOWN] = ((PORTA) & (1 << (KEY_DOWN)));
    PORTA_current[KEY_LEFT] = ((PORTA) & (1 << (KEY_LEFT)));

    if (PORTA_current[KEY_RIGHT] && !PORTA_previous[KEY_RIGHT])
        move_right();
    if (PORTA_current[KEY_UP] && !PORTA_previous[KEY_UP])
        move_up();
    if (PORTA_current[KEY_DOWN] && !PORTA_previous[KEY_DOWN])
        move_down();
    if (PORTA_current[KEY_LEFT] && !PORTA_previous[KEY_LEFT])
        move_left();

    for (char i = 0; i < ARROW_KEY_COUNT; i++) {
        PORTA_previous[i] = PORTA_current[i];
    }
}
